...
public class PromiseTaskTime implements KeepAliveComponent {
    ...
    public static String nameExpiredManager = "PromiseTaskRetry";

    private final Expired<PromiseTask> retryDelay;

    public PromiseTaskTime(ApplicationContext applicationContext) {
        @SuppressWarnings("unchecked")
        ExpiredManager<PromiseTask> expiredManager = applicationContext.getBean(ExpiredManager.class);
        retryDelay = expiredManager.get(nameExpiredManager);
        retryDelay.setOnExpired(this::onPromiseTaskRetry);
    }

    private void onPromiseTaskRetry(ExpiredMsImmutableEnvelope<PromiseTask> env) {
        env.getValue().start();
    }

    public void addRetryDelay(PromiseTask promiseTask) {
        retryDelay.add(new ExpiredMsImmutableEnvelope<>(promiseTask, promiseTask.getRetryDelayMs()));
    }
